...
**文档状态：** 产品、信息架构、内容责任与发布门禁已按 D-106 / D-107 对齐；个人正式内容仍需逐条批准
...
| 项目 | 状态 |
|---|---|
| 技术骨架 | Astro 5 静态站可构建；当前页面仍是 D-105 生产基线 |
| v2.1 设计规范 | 已确认，已归位到 `docs/design/` |
| 开场素材 | A0-H 可开始；A0-V 生产 Sprite 尚未交付 |
| `/lab/intro` | 未实现；按双轨计划先用显式占位模式开发 |
...
- 正式页面使用真实 HTML、语义链接和稳定 URL，不把关键信息烤进图片。
- 用户个人事实、经历、介绍和正式内容不得由 AI 虚构。
- 本地 harness 不生成、补画或重绘生产人物、小狗和球；只消费固定 Manifest。
- 阶段 A1 只实现桌面 `/lab/intro`，不顺手重写完整首页或内容系统。
...
2. 以 Blog 主文章作为首屏唯一内容主角；
3. 提供五个一级入口；
4. 按 Blog → Photos → Shelf → Projects → About → Footer 组织后续内容；
5. 接住开场最终人物与小狗落点，但不依赖动画才能阅读。

### 功能要求

- **FR-HOME-01：** 首屏左侧为身份区，右侧为一个 Blog 主视觉，底部只露出 Photos 下一分区 8%–12%。
...
- **FR-HOME-06：** 手机端按同一语义顺序重新排版，不等比缩小桌面拼贴。
- **FR-HOME-07：** 无 JavaScript、reduced motion 或开场素材失败时直接显示稳定首页。

## 4.2 开场实验页 `/lab/intro`

### 页面任务

只验证八拍滚动叙事、素材接口、反向滚动和最终首页落点；不承担完整首页开发。

### 功能要求

- **FR-INTRO-01：** 基准视口 1440×900，约 300vh 外层滚动与 100svh sticky 舞台。
...
详细进度与模块接口见 `docs/intro/animation-contract.md`。
...
在 1440×900 依次验证 0%、8%、25%、45%、65%、82%、94%、100%：第一拍只有球；狗与人物依次进入；人物从草地右侧离开并从首页左侧进入；最终人物与小狗完全静止。反向滚动回到每个节点时状态确定一致。
